...
static const char *usages[] = {"[OPTIONS] [-- command [args ...]]",};
...
xdrawcursor(int cx, int cy, Glyph g, int ox, int oy, Glyph og)
{
	...
	if (IS_SET(MODE_FOCUSED)) {
		...
		case 1: /* Blinking block (default) */
			if (IS_SET(MODE_BLINK) && !LOD_curtype)
				break;
			/* FALLTHROUGH */
		case 2: /* Steady block */
			xdrawglyph(g, cx, cy);
...
void
create_config(const char *path)
{
	FILE *file = fopen(path, "w");
	if (file == NULL)
		die("fopen: %s\n", strerror(errno));

	for (size_t i = 0; i < sizeof(st_json) / sizeof(*st_json); ++ i)
		fprintf(file,  "%s\n", st_json[i]);
...
json_load(json_obj_t *obj, const char *key, const char *path, json_type_t type)
{
	...
	if (ret == NULL)
		...
		die("%s: %s: Expected %s, got %s\n", path, key,
		    json_type_to_str(type), json_type_to_str(ret->type));
...
load_config(void)
{
	char path[ST_JSON_PATH_MAX];
	if (ST_JSON_PATH[0] == '~' && ST_JSON_PATH[1] == '/') {
		const char *home = getenv("HOME");
		if (home == NULL)
			die("getenv: Failed to get $HOME\n");

		strcpy(path, home);
		strcat(path, ST_JSON_PATH + 1);
	} else
		strcpy(path, ST_JSON_PATH);
...
#define JSON_LOAD_COLOR(NAME) colorname[i ++] = JSON_LOAD_STR(colors, NAME, path)
		...
		JSON_LOAD_COLOR("red");
		JSON_LOAD_COLOR("green");
		JSON_LOAD_COLOR("yellow");
		JSON_LOAD_COLOR("blue");
		JSON_LOAD_COLOR("magenta");
		JSON_LOAD_COLOR("cyan");
		JSON_LOAD_COLOR("white");

		JSON_LOAD_COLOR("grey");
		JSON_LOAD_COLOR("bright-red");
		JSON_LOAD_COLOR("bright-green");
		JSON_LOAD_COLOR("bright-yellow");
		JSON_LOAD_COLOR("bright-blue");
		JSON_LOAD_COLOR("bright-magenta");
		JSON_LOAD_COLOR("bright-cyan");
		JSON_LOAD_COLOR("bright-white");
	}
}
...
usage(FILE *file)
{
	args_usage_fprint(file, argv0, usages, sizeof(usages) / sizeof(*usages), NULL, true);
}
